...
PokemonList::PokemonList(QObject* parent, QString type, QString lang) :
...
int PokemonList::pokeNum() {
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (line.split(",")[0].toInt() < m_numberOfPokemon + 1
					&& line.split(",")[1] == m_typeIndex) {
				count++;
			}
		}
	}
...
Pokemon* PokemonList::getPokemonInfo() {
	...
	if (fil.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (i < m_numberOfPokemon && m_langIndex != "9"
					&& line.split(",")[1] == "9"
					&& pokemon[i].lang_check == false) {
	...
	if (m_typeIndex != "0") {
		for (i = 0; i < m_numberOfPokemon; i++) {
			if (pokemon[i].type_id_1 == m_typeIndex
					|| pokemon[i].type_id_2 == m_typeIndex) {
		...
		while (!in.atEnd()) {
			...
			if (j < numberOfThisType && line.split(",")[0] == pokemon[j].id) {
				...
				if (line.split(",")[1] == "1")
					...
					pokemon[j].total_stats = QString::number(
							pokemon[j].special_attack.toInt()
									+ pokemon[j].speed.toInt()
									+ pokemon[j].special_defense.toInt()
									+ pokemon[j].attack.toInt()
									+ pokemon[j].defense.toInt()
									+ pokemon[j].hp.toInt());
					j++;
				}
			}
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (j < numberOfThisType && line.split(",")[0] == pokemon[j].id) {
				...
				pokemon[j].height = QString::number(
						(line.split(",")[3]).toFloat() / 10);
				pokemon[j].weight = QString::number(
						(line.split(",")[4]).toFloat() / 10);
	...
	if (iy.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (j != 0 && j - 1 < numberOfThisType
					&& line.split(",")[0] == pokemon[j - 1].id
					&& line.split(",")[2] == "0" && line.split(",")[3] == "2") {
	...
	if (ile.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (j < numberOfThisType && pokemon[j].version_check == false
					&& line.split(",")[0] == pokemon[j].id
					&& (m_langIndex == line.split(",")[2]
							|| (m_langIndex != "5" && "9" == line.split(",")[2]))) {
				...
				for (int i = 3; i < line.split(",").length(); i++) {
					if (i == 3)
						...
						pokemon[j].description = pokemon[j].description + ", "
								+ line.split(",")[i];
				}
			...
			if (j != 0 && j - 1 < numberOfThisType
					&& line.split(",")[0] == pokemon[j - 1].id
					&& pokemon[j - 1].version_check == false) {
...
Pokemon PokemonList::getTypeNames(Pokemon pokemon) {
	...
	if (fil.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (line.split(",")[0] == type1.type
					&& line.split(",")[1] == m_langIndex) {
				pokemon.type1 = line.split(",")[2];
				type1.lang_check = true;
			}

			if (m_langIndex != "9" && line.split(",")[1] == "9"
					&& type1.lang_check == false
					&& line.split(",")[0] == type1.type)
				pokemon.type1 = line.split(",")[2];
			// Gets the type in english if the selected language isn't there

			if (line.split(",")[0] == type2.type
					&& line.split(",")[1] == m_langIndex) {
			...
			if (m_langIndex != "9" && line.split(",")[1] == "9"
					&& type2.lang_check == false
					&& line.split(",")[0] == type2.type)
				pokemon.type2 = line.split(",")[2];
			// Gets the type in english if the selected language isn't there
	...
	if ((pokemon.id).toInt() < 10)
		pokemon.names = "#" + ("00" + pokemon.id) + " " + pokemon.names + " ["
				+ pokemon.types + "]";

	else if ((pokemon.id).toInt() < 100)
		pokemon.names = "#" + ("0" + pokemon.id) + " " + pokemon.names + " ["
				+ pokemon.types + "]";

	else
		pokemon.names = "#" + pokemon.id + " " + pokemon.names + " ["
				+ pokemon.types + "]";
...
Pokemon PokemonList::getAbilityAndVersionNames(Pokemon pokemon) {
	...
	pokemon.version = pokemon.version.split(" ")[0] + " "
			+ pokemon.version.split(" ")[1] + " "
			+ pokemon.version.split(" ")[2];
	...
	if (li.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (line.split(",")[0] == pokemon.ability
					&& m_langIndex == line.split(",")[1]) {
				pokemon.ability = line.split(",")[2];
				langCheck = true;
			}

			if (line.split(",")[0] == pokemon.ability
					&& line.split(",")[1] == "9" && langCheck == false)
				pokemon.ability = line.split(",")[2];

			if (line.split(",")[0] == pokemon.ability_2
					&& m_langIndex == line.split(",")[1]) {
			...
			if (line.split(",")[0] == pokemon.ability_2
					&& line.split(",")[1] == "9" && langCheck_2 == false)
				pokemon.ability_2 = line.split(",")[2];
		}
	}
...
QString PokemonList::statLang(QString statIndex) {
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		while (!in.atEnd()) {
			...
			if (line.split(",")[0] == statIndex
					&& line.split(",")[1] == m_langIndex) {
				statName = line.split(",")[2] + ":";
				language_check = false;
			}

			else if (line.split(",")[1] == "9" && language_check
					&& line.split(",")[0] == statIndex)
				statName = line.split(",")[2] + ":";
