...
void ApplicationUI::typeSelected(int type) {
    ...
    if (status) { // did we succeed in getting a pointer to the Label UI control?
    	...
    	status->setText(QString("Found %1 Pokemon").arg(m_pokemonList->childCount(QVariantList())));
    }
    else {
	...
	// Populate the dropdown list of types
	    ...
	    int i = 0;
	    	...
	    	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
	    		...
	    		while (!in.atEnd()) {
	    			...
	    			if (i < 18 && line.split(",")[1] == "9" && type_list[i].lang_check == false)
	    	...
	    	if (m_pokemonList->getType() == "0" && (DefaultLang == "9" || m_pokemonList->getLang() == "9"))
	    		dropDown->add(Option::create().text("All Types").value(0).selected(true));

	    	else if (m_pokemonList->getType() == "0" && (DefaultLang == "1" || m_pokemonList->getLang() == "1"))
	    	{
	    		...
	    		if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
	    			...
	    			while (!in.atEnd()) {
	    				QString line = in.readLine();
	    				dropDown->add(Option::create().text(line).value(0).selected(true));
	    			}
	    		}
	    		else {
	    			cerr <<"Failed to open japanese_all_types.cvs: " << file.error() << endl;
	    			exit(0);
	    		}
	    	}
	    	...
	    	if (dropDown) { // did we succeed in getting a pointer to the drop down UI control?
	    	    	for (i = 0; i < 18; i++)
	    	    	{
	    	    		if (m_pokemonList->getType() == type_list[i].type)
	    	    			dropDown->add(Option::create().text(type_list[i].type).value(i+1).selected(true)); //Adds each type with a different corresponding value from int i

	    	    		else
	    	    			dropDown->add(Option::create().text(type_list[i].type).value(i+1));
	    	    	}
	    	}
	    ...
	    QFile file("app/native/assets/data/language_names.csv");
	    	if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
	    		...
	    		while (!in.atEnd()) {
	    				QString line = in.readLine();
	    				if (line.split(",")[1] == "9" && line.split(",")[0] == DefaultLang && StartUp)  {
	    					radio->add(Option::create().text(line.split(",")[2]).value(line.split(",")[0]).selected(true));
	    				}
	    				if (line.split(",")[1] == "9" && (line.split(",")[0] != DefaultLang || !StartUp))  {
	    					radio->add(Option::create().text(line.split(",")[2]).value(line.split(",")[0]));
	    				}
	    			}
	    		}
	    	else {
	    		cerr <<"Failed to open language_names.cvs: " << file.error() << endl;
	    		exit(0);
	    	}
	    }
